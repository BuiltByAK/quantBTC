...
miner_cap <- c("SplyMiner0HopAllNtv", #Supply from direct block makers
               "SplyMiner1HopAllNtv", #Supply from mining pools
...
df <- df %>%
  mutate(
    miner_cap = SplyMiner0HopAllNtv + SplyMiner1HopAllNtv,
...
metrics_chart <- df %>% ggplot(aes(x = time)) +
  ...
  geom_line(aes(y = rctc_scaled_mult, col = "RCTC"), size = 0.3, alpha = 0.5) +
  geom_line(aes(y = NVTAdjFF_scaled, col = "NVTAdjFF"), size = 0.3, alpha = 0.4) +
  geom_line(aes(y = rvt_scaled, col = "RVT"), size = 0.3, alpha = 0.4) +
  geom_line(aes(y = rvtadj90_scaled, col = "RVT_90"), size = 0.3, alpha = 0.4)
