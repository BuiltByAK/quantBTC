...
df_pre2018 <- df_pre2018 %>%
  mutate(expected_sharpe = predicted_ROI30d / garch,
         position = case_when(expected_sharpe > 0.1 ~ 1, #Go long
                              expected_sharpe < -0.1 ~ -1, #Go Short
                              TRUE ~ 0 #Neutral
                              ),
  ...
  dplyr::select(date, forward_ROI30d, predicted_ROI30d, expected_sharpe,cum_strategy_return, cum_buy_hold_return, position, PriceUSD, AdrActCnt, TxCnt, TxTfrValAdjUSD,
  FeeTotUSD, RevUSD, VelCur1yr, btc_dominance,
  SplyAdrBalNtv1K, SplyFF, garch)


df_pre2018 %>% ggplot(aes(x = date)) +
...
```{r}
formula_enhanced <- forward_ROI30d ~ AdrActCnt + TxCnt +
  FeeTotUSD + RevUSD + btc_dominance + SplyAdrBalNtv1K

formula_enhanced1 <- forward_ROI30d ~ AdrActCnt + TxCnt +
  FeeTotUSD + RevUSD + btc_dominance + SplyAdrBalNtv1K + garch
...
df_final <- df_final %>% dplyr::select(
...
training_model <- df_final_training %>% lm(formula = formula_enhanced)
#summary(training_model) 
#vif(training_model)

df_final_training <- df_final_training %>%
  mutate(residuals = resid(training_model))

garch_input_final_training <- df_final_training %>%
  select(date, residuals) %>%
  rename(series = residuals)

garch_output_final_training <- RTL::garch(x = garch_input_final_training, out = "data")

garch_df <- garch_output_final_training %>%
  data.frame(date = index(.), garch_vol_resid = coredata(.)) %>%
  as_tibble()

df_final_training <- df_final_training %>%
  left_join(garch_df, by = "date")

training_model_garch <- df_final_training %>% lm(formula = formula_enhanced1)
#summary(training_model_garch)
#vif(training_model_garch)

library(slider)

# === STEP 1: Define your ranges ===
roll_window <- 90  # Keep window size fixed (or loop over this too)
mean_multipliers <- seq(0.2, 1.0, by = 0.1)
sd_multipliers <- seq(0.5, 2.0, by = 0.1)

# === STEP 2: Grid search for best multiplier combo ===
roll_opt_results <- expand_grid(mean_mult = mean_multipliers,
                                sd_mult = sd_multipliers) %>%
  mutate(
    sharpe = pmap_dbl(list(mean_mult, sd_mult), function(mean_mult, sd_mult) {
      df_temp <- df_final_training %>%
        mutate(
          expected_sharpe = predict(training_model, newdata = .) / garch,
          roll_mean = slide_dbl(expected_sharpe, mean, .before = roll_window, .complete = TRUE),
          roll_sd   = slide_dbl(expected_sharpe, sd,   .before = roll_window, .complete = TRUE),
      ...
      # Return Sharpe Ratio
      mean(df_temp$strategy_return, na.rm = TRUE) /
        sd(df_temp$strategy_return, na.rm = TRUE)
    })
  )

# === STEP 3: Get best combo ===
best_roll_combo <- roll_opt_results %>%
  filter(sharpe == max(sharpe, na.rm = TRUE))

print(best_roll_combo)
...
garch_input_live <- df_final_live %>%
  select(date, forward_ROI30d) %>%
  rename(series = forward_ROI30d)

garch_output_live <- RTL::garch(x = garch_input_live, out = "data")

garch_df_live <- garch_output_live %>%
  data.frame(date = index(.), garch = coredata(.)) %>%
  as_tibble()

df_final_live <- df_final_live %>%
  left_join(garch_df_live, by = "date")

df_final_live <- df_final_live %>%
  mutate(predicted_ROI30d = predict(training_model, newdata = df_final_live),
         expected_sharpe = predicted_ROI30d / garch,
         roll_mean = slide_dbl(expected_sharpe, mean, .before = window_size, .complete = TRUE),
         roll_sd = slide_dbl(expected_sharpe, sd, .before = window_size, .complete = TRUE),
         dynamic_long_thresh = roll_mean + best_roll_combo$mean_mult * roll_sd,
         dynamic_short_thresh = roll_mean - best_roll_combo$sd_mult * roll_sd,
         position_dynamic = case_when(
           expected_sharpe > dynamic_long_thresh ~ 1,
           expected_sharpe < dynamic_short_thresh ~ -1,
           TRUE ~ 0
         ),
         strategy_return_dynamic = lag(position_dynamic) * forward_ROI30d / 30, 
         cum_strategy_return_dynamic = cumsum(replace_na(
           strategy_return_dynamic, 0)),
         daily_returns = (PriceUSD / lag(PriceUSD) - 1) * 100,
         strategy_return_daily = lag(position_dynamic) * daily_returns,
         cum_strategy_return_daily = cumsum(replace_na(strategy_return_daily, 0)),
         cum_daily_returns = cumsum(replace_na(daily_returns, 0)),
         cum_buy_hold_return = cumsum(replace_na(daily_returns, 0))
  )
...
strategy_plot <- df_final_live %>%
  ...
  geom_line(aes(y = cum_strategy_return_daily, color = "Daily Returns on Strategy")) +
  labs(
    title = "Dynamic Sharpe Threshold Strategy vs Buy & Hold (Live 2023+)",
    y = "Cumulative ROI(%)",
    color = "Legend"
  ) +
  theme_minimal()
...
# Drawdown helper function
calc_drawdown <- function(returns) {
  cum_returns <- cumsum(replace_na(returns, 0))
  max_cum <- cummax(cum_returns)
  drawdown <- cum_returns - max_cum
  return(drawdown)
}

# Add drawdowns to live data
df_final_live <- df_final_live %>%
  mutate(
    drawdown_strategy = calc_drawdown(strategy_return_daily),
    drawdown_buyhold = calc_drawdown(daily_returns)
  )
...
drawdown_plot <- df_final_live %>%
  ...
  labs(
    title = "Drawdown Comparison: Strategy vs Buy & Hold",
    y = "Drawdown (%)",
    color = "Legend"
  ) +
  theme_minimal()
